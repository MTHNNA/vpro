    ...
    async def extract(self, url: str) -> Optional[VideoResult]:
        ...
        if images:
            ...
            for idx, img in enumerate(images, 1):
                ...
                img_url  = next((u for u in url_list if ".jpeg?" in u), None) or (url_list[0] if url_list else None)
                if img_url:
                    items.append(MediaItem(url=img_url, filename=f"{filename_base}_slide_{idx}.jpg", is_video=False))
            if items:
                return VideoResult(
                    url=items[0].url,
